...
router.get('/qr-token', requireRole('company'), async (req: Request, res: Response, next: NextFunction) => {
  try { res.json(await Service.getQrToken(req.user!.sub)); } catch (e) { next(e); }
});

// POST /attendance/checkin
router.post('/checkin', requireRole('worker'), validate(QrScanSchema), async (req: Request, res: Response, next: NextFunction) => {
  try { res.json(await Service.checkin(req.user!.sub, req.body.qr_token)); } catch (e) { next(e); }
});

// POST /attendance/checkout
router.post('/checkout', requireRole('worker'), validate(QrScanSchema), async (req: Request, res: Response, next: NextFunction) => {
