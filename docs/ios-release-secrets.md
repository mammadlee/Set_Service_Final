...
| APNs auth | `<APNS_AUTH_KEY>` / Key ID / Team ID in Firebase Console or approved provider, not the repository | Enable Push Notifications on the App ID. The signed provisioning profile/entitlements must provide the matching `aps-environment` value (`development` or `production`) for the build; never guess or hardcode it. Test device token, refresh, notification delivery and logout token removal on a real iPhone |
